-- =============================================================
-- Restyling modale satellite Campionatura (vedi CLAUDE.md — mappatura
-- campi Campionatura, sullo stesso template di Progetto/Acconto): semaforo
-- ridisegnato, SOSTITUISCE quello del 2/8 (migration 0030) — non più
-- derivato da `descrizione` ma da `campione_data_consegna`, ora un campo
-- Data liberamente editabile dall'utente (come acconto_data), non più un
-- side-effect "leggi poi scrivi" legato alla transizione di
-- campione_consegnato (comportamento applicativo rimosso in
-- aggiornaCampione(), lib/lavori/satelliti.ts). Rosso finché la Data non è
-- valorizzata, giallo quando la Data è presente ma non ancora consegnato,
-- verde quando consegnato=true (invariato, priorità massima).
--
-- lavoro_pronto_per_montaggio() NON viene toccata: la sua unica condizione
-- per 'campione' è già `not coalesce(campione_consegnato, false)`, mai
-- dipesa da `descrizione` — nessun cambiamento necessario lì, il gate
-- montaggio blocca/sblocca esattamente come prima.
--
-- Verificato su Supabase Cloud prima di scrivere questa migration (come
-- da prassi per ogni cambio di semaforo con dati storici reali, vedi
-- CLAUDE.md): 6 righe tipo='campione' esistenti. 4 invariate (2 già rosso
-- restano rosso, 2 già verde restano verde — verde dipende solo da
-- campione_consegnato, mai toccato qui). 2 righe cambiano da giallo a
-- rosso: avevano `descrizione` compilata ma nessuna `campione_data_consegna`
-- (mai valorizzata, essendo consegnato=false su entrambe) — conseguenza
-- accettata, nessun backfill possibile/sensato (nessuna data equivalente
-- da cui derivarla), stesso principio già accettato per altri cambi di
